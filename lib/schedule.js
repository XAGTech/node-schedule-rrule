...
Job.prototype.schedule = function(spec) {
  ...
  const shouldIncludeStart =
    this.recurrenceRule.hasExplicitStart && this.recurrenceRule.start.getTime() >= now.getTime();

  const inv = scheduleNextRecurrence(self, start, end, shouldIncludeStart);
  if (inv !== null) {
...
function scheduleNextRecurrence(job, prevDate, endDate, inclusive) {
  prevDate = prevDate || new Date();
  inclusive = inclusive === true;

  job.recurrenceRule.safelyResetStartDate();
  const date = job.recurrenceRule.nextInvocationDate(prevDate, inclusive);
